...
void cfbMenu(){
   ...
   std::cout << "      HttpGet:\n";
   std::cout << "          Usage: ./HackTool [-HTTPg] [-D domain]\n";
   std::cout << "      HttpPost:\n";
   std::cout << "          Usage: ./HackTool [-HTTPp] [-D domain message]\n\n";
...
int main(int argc, char *argv[]){
   ...
   if (std::string(argv[1]) == "-HTTPg") {
      if (std::string(argv[2]) ==  "-D") {
         mainMenuHttpGet(argv[3]);
      }
   }
   if (std::string(argv[1]) == "-HTTPp") {
      if (std::string(argv[4]) == "-D") {
         mainMenuHttpPost(argv[5],argv[6]);
      }
   }
